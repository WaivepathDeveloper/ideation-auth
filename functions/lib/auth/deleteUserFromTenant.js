 ...
 */
exports.deleteUserFromTenant = functions.https.onCall(async (data, context) => {
    // Authentication check
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be authenticated to delete users');
    }
    // Rate limiting
    ...
    if (context.auth.token.role !== 'tenant_admin') {
        throw new functions.https.HttpsError('permission-denied', 'Only tenant administrators can delete users');
    }
    ...
    if (user_id === context.auth.uid) {
        throw new functions.https.HttpsError('permission-denied', 'You cannot delete yourself. Transfer admin privileges first.');
    }
...
//# sourceMappingURL=deleteUserFromTenant.js.map